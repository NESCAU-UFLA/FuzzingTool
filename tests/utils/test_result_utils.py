...
class TestResultUtils(unittest.TestCase):
    ...
    def test_format_data_field(self):
        return_expected = "True"
        returned_data = ResultUtils.format_data_field(True)
        self.assertIsInstance(returned_data, str)
        self.assertEqual(returned_data, return_expected)

    def test_format_data_field_with_detailed_list(self):
        return_expected = "test, test2"
        returned_data = ResultUtils.format_data_field(["test", "test2"], force_detailed=True)
        self.assertIsInstance(returned_data, str)
        self.assertEqual(returned_data, return_expected)

    def test_format_data_field_with_list(self):
        test_custom = ["test", "test2"]
        return_expected = f"found {len(test_custom)} match(s)"
        returned_data = ResultUtils.format_data_field(test_custom)
        self.assertIsInstance(returned_data, str)
        self.assertEqual(returned_data, return_expected)
