...
def assert_payload_list_is_equal(payloads: List[Payload], other_payloads: List[Payload]) -> None:
    ...
    for i, payload in enumerate(payloads):
        ...
        assert payload.rlevel == other_payloads[i].rlevel
...
class TestPayloader(unittest.TestCase):
    ...
    def test_get_customized_payload_with_prefix(self):
        ...
        test_prefix = ['<', '|']
        return_expected = [
            Payload().update(test_payload).with_prefix(test_prefix[0]),
            Payload().update(test_payload).with_prefix(test_prefix[1])
        ]
    ...
    def test_get_customized_payload_with_suffix(self):
        ...
        test_suffix = ['>', '|']
        return_expected = [
            Payload().update(test_payload).with_suffix(test_suffix[0]),
            Payload().update(test_payload).with_suffix(test_suffix[1])
        ]
    ...
    def test_get_customized_payload_with_upper(self):
        test_payload = Payload("test_payload")
        return_expected = [test_payload.with_case(str.upper, "Upper")]
    ...
    def test_get_customized_payload_with_lower(self):
        test_payload = Payload("test_payload")
        return_expected = [test_payload.with_case(str.lower, "Lower")]
    ...
    def test_get_customized_payload_with_capitalize(self):
        test_payload = Payload("test_payload")
        return_expected = [test_payload.with_case(str.capitalize, "Capitalize")]
